...
export const Categorias = () => {
	...
	async function DesAtivar(arg) {
		try {
			...
			const response = await fetch(`${route}/tipoitens/${arg.target.id}`, {
				method: "PUT",
				mode: "cors",
				headers: {
					...
					{tipoItens.map((categoria,index) => {
						return (
							<Card.Root key={index} colNumber={4} hover={true}>
								...
								<Card.Body>
									<Card.Buttons>
										<Card.Button action={DesAtivar} id={categoria.id} title={`${categoria.ativo?"Desativar":"Reativar"} categoria`}/>
									</Card.Buttons>
								</Card.Body>
							</Card.Root>
						);
